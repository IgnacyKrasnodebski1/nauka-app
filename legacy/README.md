...
## Nauka 2.0 — krok 8: gamifikacja (design/DESIGN.md §4.5, §6 pkt 8) — gemy, plecak, misje, seria, album, boss, duch, odznaki

Wszystko offline, w tym samym kluczu `nauka_progress_v1` (nowe pola dopisane, bez migracji); seria nadal w `nauka_meta_v1`.

- **Gemy** (`PROGRESS.gems`, `gems()/addGems(n, why)` z toastem `.a-pop`): +10 za zaliczony poziom (+5 za 3 gwiazdki, `GEM`), +10 skrzynia, +20–30 misja dzienna / +100 tygodniowa, +5 cały plan dnia (`planCheck` po każdym `completeDaily`), +25 boss (pierwsze zwycięstwo). Pigułka gemów (`gemPill`, `#gemNum`) w pasku Dziś (Main.html) i w pasie przedmiotu obok serc; tap → Plecak.
- **Skrzynie na ścieżce**: co trzeci poziom (poza ostatnim), otwiera się, gdy trzy poprzednie poziomy są zaliczone (`.a-pop` + toast + gemy); otwarte trzymane w `PROGRESS[sid].chests["<indeks>"]` (węzeł `.node-chest.opened`, etykieta „Otwarta”).
- **Plecak** (`renderShop`, widok `shop`, Shop.html): Zamrożenie serii (100, max 2 w `META.freezes`), Uzupełnienie serc (50), Podwójne XP na 15 min (80, `PROGRESS.boost.until`; `addXP` podwaja dodatnie XP, chip „×2 XP” w sesji), Motyw (150; `PROGRESS.themes` posiadane, `PROGRESS.theme` aktywny — kolor rozmytego koła na Dziś przez `.blob.th-*`, chipy posiadanych zestawów pod wierszem). Stany przycisku: cena (`.buy`), za mało (`.buy.short` + toast „Brakuje N gemów”), komplet (`.buy.owned`). Karta „Skąd brać gemy”. Definicje w `SHOP[]` (`sub()/can()/buy()`).
- **Misje** (`renderMissions`, widok `missions`, Missions.html): 3 dzienne wybierane deterministycznie z daty (`hashStr`, pula `MISSION_POOL`: zdobądź N XP, N poprawnych z rzędu, powtórz N pojęć, zalicz poziom, zadanie na czas bez błędu, cały plan dnia; cel losowany z `targets`) + 1 tygodniowa (`WEEKLY_POOL`: 5 poziomów / 300 XP / 5 dni nauki, klucz = poniedziałek tygodnia). Stan `PROGRESS.missions={date, daily:[{id,progress,target,reward,done}], weekly:{week,id,…}}`. Postęp przez `missionEvent(kind, n)` z: `addXP` (xp), combo w lekcji (streakok, max), `rvAnswer` i fiszki (review), `finishLesson` (level), zadanie `tf` z `seconds` (tftime), `planCheck` (plan), `touchStreak` (day). Gotowa misja = przycisk **Odbierz** (`claimMission` → gemy, `stats.missions`); wiersz zrobiony w limonkowym tincie; „zostało X h Y min” do północy. Kafel „Misje x/3” na Dziś (`#missionTile`) otwiera ekran.
- **Seria**: `META.freezes` (0–2). Na starcie `applyFreeze()`: opuszczone dni pokryte zamrożeniami → seria zostaje (`lastDay` = wczoraj, dni w `META.frozenDays`, toast); za mało zamrożeń → seria przepada jak dotąd. Ekran Seria: dzień uratowany zamrożeniem jako cyjanowa kropka (`.day.frozen`), karta **Zamrożenie serii** („Masz N z 2 — jedno poszło wczoraj”, → Plecak).
- **ComeBack** (`renderComeBack`, widok `comeback`, ComeBack.html): na starcie, gdy `lastDay` ≥ 3 dni temu (raz dziennie, `META.comebackShown`): „Nie było cię N dni”, ciepły tekst bez wyrzutów, karta „Seria zaczyna się od nowa” (stara → 0), „Czeka na powtórkę” (zaległe SRS per przedmiot) albo „Nic nie przepadło”, karta „Zacznij od 5 minut”; **ZACZNIJ OD 5 MINUT** → sesja powtórki (max 15 zaległych) albo pierwszy wiersz planu dnia; „NORMALNY PLAN DNIA” → Dziś.
- **Album pojęć** (`renderAlbum`, widok `album`, Album.html): fiszka trafia do `PROGRESS.album["<sid>:<lid>:<idx>"]={rarity, at}` w `srsTouch`, gdy pudełko SRS ≥ 3 (`albumCheck`); rzadkość: `epic` (poziom zaliczony na 3 gwiazdki), `rare` (zebrana bez pomyłek, `lapses` 0), `common`. Ekran: „N z M zebrane” + pasek, chipy przedmiotów (z liczbą), siatka 3 kolumn — zebrane (najnowsze pierwsze, plakietka NOWA z dzisiaj, tap = definicja `.alb.flip`), zablokowane jako kreskowane sylwetki z kłódką; legenda zwykłe / trudne / mistrzowskie. Wejścia: kafel „Album N” na Dziś, „Odznaki — Album pojęć ›” w Profilu.
- **Boss rozdziału** (`startBoss/renderBoss/finishBoss`, Boss.html): ostatni węzeł ścieżki `.node-boss` (potwór jako inline SVG w kolorze przedmiotu, `bossSvg`; zablokowany do zaliczenia wszystkich poziomów, potem `.open` z dymkiem WALKA, po wygranej `.beaten` z checkiem). Walka na pełnym ekranie (`#lesson.bossrun`): nagłówek z ucieczką i sercami, potwór z paskiem życia (HP = min(10, pula) — pytania quizu i zadania całego przedmiotu, `allQuiz + allTasks`), „Cios N”, **20 s na pytanie** (`.bosstime`, pod 5 s czerwień + `.a-blink`; koniec czasu = pudło z pokazaną poprawną). Trafienie = −1 HP, `.a-shake` na potworze i chip „−1”, +5 XP; pudło = −1 serce + ramka „Zapamiętaj”. Wygrana: +50 XP, +25 gemów, odznaka (tylko pierwsza; kolejne +15 XP), `PROGRESS[sid].boss={done,n,at,best}`; przegrana (0 serc) = ekran z „SPRÓBUJ PONOWNIE” (bez serc → panel Koniec żyć). Nazwa: `subject.boss` (opcjonalne) albo „Strażnik: <przedmiot>”.
- **Duch** (Ghost.html): przebieg poziomu `lessonState.run=[{t, correct}]` (ms od pierwszego pytania). Zaliczony poziom bez ducha zapisuje się jako `PROGRESS.ghost["<sid>:<lid>"]={run, at}`; przy powtórce nad pytaniem karta `.ghostcard`: pasek „Ty” i pasek „Ty z <dnia>” (pozycja ducha z jego znaczników czasu, `ghostTick` co 400 ms), notka „Prowadzisz o X s” / „Duch prowadzi” / „Łeb w łeb” (+ gdzie duch się pomylił). Wynik: lepszy przebieg (więcej trafień, przy remisie szybszy) = **+10 XP** i nowy duch (`ghostFinish`); wiersz `.lcghost` na LevelComplete.
- **Odznaki** (`BADGES`, `checkBadges` po każdym istotnym zdarzeniu, `PROGRESS.badges={id:date}`): Pierwszy poziom, 7 dni serii, 30 dni serii, 100 pojęć w albumie, Egzamin 90%, Boss pokonany, 10 misji, Combo ×3 (10 poprawnych z rzędu), Plan dnia 5 razy, Cały przedmiot. Liczniki w `PROGRESS.stats={missions,maxCombo,planDays,bosses}`. Odblokowanie = toast z ikoną `.a-pop`; Profil: siatka 3 kolumn (`badgeGrid`, `#badgegrid`) z zablokowanymi w kreskowanej ramce.
- **Koniec żyć** (NoHearts.html): druga karta **„Uzupełnij wszystkie — 50 gemów · zostanie N”** (`REFILL_GEMS`; za mało → karta wyszarzona i toast); po uzupełnieniu w trakcie lekcji gra toczy się dalej (`showNoHearts({onContinue})`), także gdy życie wróci samo.
- Nowe ikony: `snow`, `palette`, `boss`, `ghost` (oczy/usta przez `.ic .eye/.mouth` w tokenach). `toast(t, ic, anim)` przyjmuje klasę animacji ikony. Reset postępów kasuje też gemy, album, odznaki (seria i zamrożenia zostają w meta).
- `styles.css`: blok „KROK 8” (`.streak.gems`, `.minitile.cyan`, `.blob.th-*`, `.shop/.shoprow/.buy/.themechips/.themechip`, `.missions/.msrows/.msrow/.claim/.msnote/.msfoot`, `.freezecard/.day.frozen`, `.comeback/.cb*`, `.album/.albhead/.albchips/.albchip/.albgrid/.alb(.rare/.epic/.lock/.flip)/.alblegend`, `.boss .b1–.b6`, `.node-boss`, `.bossrun/.bosshero/.bosshp/.bossq/.bosstime/.bossexp`, `.lcbig.boss/.lcrewards/.lcreward`, `.ghostcard/.ghrow/.ghnote/.lcghost`, `.badgegrid/.badge.acid/.cyan/.violet/.pink`, `.nhcard.cyan`). Zero nowych tokenów i zero literałów kolorów.
