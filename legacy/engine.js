...
function allCards(s){return s.levels.flatMap(l=>(l.flashcards||[]).map(c=>({...c,lvl:noEmoji(l.title)})));}
function allQuiz(s){return s.levels.flatMap(l=>(l.quiz||[]).map(q=>({...q,lvl:noEmoji(l.title)})));}
...
function renderLesson(){
  ...
  if(ls.phase==='feed'){
    ...
    card.innerHTML=`<div class="fin"><span class="tag accent">Mikro-dawka ${ls.feedIdx+1}/${totalFeed}</span>
      ...
      ${f.real?`<div class="real a-up d2"><span class="lbl">${icon('bulb',{size:15})}po ludzku</span>${f.real}</div>`:''}
...
function finishLesson(){
  ...
  lc.innerHTML=`<div class="lcstars">${starIc(34,'a-pop d1',stars>=2)}${starIc(46,'a-pop',stars>=1)}${starIc(34,'a-pop d3',stars>=3)}</div>
    ...
    ${passed?'':'<p class="lcp">Poniżej 50% — przejedź roladkę jeszcze raz i spróbuj ponownie, dasz radę.</p>'}`;
...
function renderFiszki(sc){
  ...
  const list = fState.lvl==='all'?allCards(s):(s.levels.find(l=>l.id===fState.lvl).flashcards||[]).map(c=>({...c,lvl:noEmoji(s.levels.find(l=>l.id===fState.lvl).title)}));
  ...
  const next=(known)=>{if(known){addXP(s.id,2);toast('+2xp','check');}tickDaily(s.id,'review',1);heartReview();fState.flipped=false;fState.idx=(fState.idx+1)%Math.max(1,list.length);renderFiszki(sc);};
