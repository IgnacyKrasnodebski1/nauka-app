...
export function PathTab({ topic }: { topic: Topic }) {
  ...
  return (
    <>
      <div className="progressrow mt-2">
        <div className="bar"><i style={{ width: `${c.pct}%` }} /></div>
        <div className="counter">{c.done}/{c.total} poziomów</div>
      </div>
      {c.done === c.total && c.total > 0 && <div className="exfb ok mb-2">Cały temat zaliczony. Zrób egzamin próbny albo wróć po gwiazdki.</div>}
      <div className="path">
        {topic.levels.map((lv, i) => {
          ...
          const inner = lp.done ? "✓" : unlocked ? lv.emoji || "📘" : "🔒";
          const label = (
            <div className="nodelabel">
              <div className={cn("t", !unlocked && "lock")}>{lv.title}</div>
              <small>{lv.quiz.length} pytań · {lv.flashcards.length} fiszek{lv.games?.length ? ` · ${lv.games.length} gry` : ""}</small>
              {lp.done && (
                <div className="stars" aria-label={`${lp.stars} z 3 gwiazdek`}>
                  {[0, 1, 2].map((k) => <span key={k} className={k < lp.stars ? "" : "off"}>★</span>)}
                </div>
              )}
            </div>
          );
          return (
            <div key={lv.id} className="pathnode">
              {unlocked ? (
              ...
              {unlocked ? <Link href={`/app/t/${topic.id}/l/${lv.id}`} className="contents">{label}</Link> : label}
            </div>
          );
        })}
      </div>
