  ...
  return (
    <div className="modal" role="dialog" aria-modal="true" aria-labelledby="sp-title">
      <div className="card">
        {step === "stage" ? (
          <>
            <span className="tag">Krok 1 z 2</span>
            <h2 id="sp-title">Na jakim etapie jesteś?</h2>
            <p className="mt-1 mb-4">Dopasujemy przedmioty, język i trudność pytań. Zmienisz to potem w koncie.</p>
            <div className="grid grid-cols-2 gap-3">
              {STAGES.map((s) => (
                <button key={s.id} type="button" onClick={() => { setLocalStage(s.id); setPicked([]); setStep("subjects"); }} className={cn("card raised hover text-left !p-4", stage === s.id && "!border-[var(--accent)]")}>
                  <div className="tile sm neutral mb-2" aria-hidden="true">{s.emoji}</div>
                  <div className="display font-bold text-[15px] text-txt">{s.label}</div>
                  <div className="text-muted text-xs mt-0.5">{s.hint}</div>
                </button>
              ))}
            </div>
          </>
          ...
          <>
            <div className="flex items-center justify-between">
              <span className="tag">{mode === "onboarding" ? "Krok 2 z 2" : "Nowy przedmiot"}</span>
              {mode === "onboarding" ? (
            ...
            <p className="mt-1 mb-4">Zaznacz przedmioty — każdy dostanie własną półkę na tematy.</p>
            ...
            <button type="button" className="pill mt-4" disabled={busy || !picked.length} onClick={save}>
              {busy ? "Zapisuję…" : picked.length ? `Dalej (${picked.length})` : "Wybierz przedmioty"}
            </button>
          </>
        )}
      </div>
