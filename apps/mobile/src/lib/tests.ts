/**
 * Plan do sprawdzianu (design/DESIGN.md §4.3, TestPlan.html) — ten sam algorytm co apps/web/src/lib/tests.ts (port legacy
 * `buildTestPlan`/`syncTests`). Poziom w zakresie = "<topicId>:<levelId>". Jeden plan na przedmiot w `user_meta.tests`.
 */
import { dayDiff, levelProgress, todayStr, type SubjectProgress, type Topic, type WeakMap } from "@nauka/shared";
import { addDays, noEmoji, npl } from "./format";
...
/** Raz dziennie: usuń plany po terminie, przelicz od dziś. Zwraca tę samą tablicę, gdy nic się nie zmieniło. */
...
/** Zalicz dzisiejszy wiersz (learn wymaga zaliczenia wszystkich poziomów wiersza). null = bez zmian. */
...
export function tpTitle(r: PlanRow, topics: Pick<Topic, "id" | "levels">[]): string {
  if (r.kind === "learn") {
    ...
    if (n === 1) {
      ...
      if (l) return "Nauka: " + noEmoji(l.title);
    }
    return `Nauka: ${npl(n, "poziom", "poziomy", "poziomów")}`;
  }
...
export function tpSub(r: PlanRow): string {
  ...
  if (r.kind === "mock") return `${r.minutes} min · ${npl(r.n ?? 0, "pytanie", "pytania", "pytań")}`;
