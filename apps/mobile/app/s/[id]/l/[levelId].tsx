import { XP, applyQuizResult, cardKey, newCard, review, type Level, type MiniGame, type QuizQuestion } from "@nauka/shared";
...
import { gameLabel, gamesForLevel, shuffle } from "@/lib/games";
...
/** Lekcja: feed → fiszki → mini-gry → quiz → wynik. Zaliczenie: quiz ≥ LEVEL_PASS (shared applyQuizResult). */
export default function LessonScreen() {
  ...
  useEffect(() => {
    let alive = true;
    if (id) app.getSubject(id).then((s) => alive && setSubject(s));
    return () => {
      alive = false;
    };
  ...
  const close = () => (router.canGoBack() ? router.back() : router.replace({ pathname: "/s/[id]", params: { id: id ?? "" } }));

  if (subject === undefined) return <Loading label="wczytuję lekcję…" />;
  if (!subject || !level)
    return (
  ...
  return (
    <AccentProvider accent={subject.accent} accent2={subject.accent2}>
      <Lesson key={level.id} subject={subject} level={level} onClose={close} />
    </AccentProvider>
  );
}

function Lesson({ subject, level, onClose }: { subject: AppSubject; level: Level; onClose: () => void }) {
  const app = useApp();
  const insets = useSafeAreaInsets();
  const games = useMemo<MiniGame[]>(() => gamesForLevel(level), [level]);
  const quiz = useMemo<QuizQuestion[]>(() => shuffle(level.quiz).slice(0, Math.min(QUIZ_N, level.quiz.length)), [level]);
  ...
  const finish = () => {
    ...
    const r = quiz.length ? applyQuizResult(app.progressFor(subject), level.id, score, quiz.length) : applyQuizResult(app.progressFor(subject), level.id, 1, 1);
    app.setProgressFor(subject, r.progress);
    setResult(r);
    setPhase("result");
  ...
  const gradeCard = (known: boolean) => {
    ...
    app.setSrsFor(subject, { ...srs, [key]: review(srs[key] ?? newCard(), known ? 3 : 0) });
    if (known) {
  ...
  const gameDone = (correct: number, tot: number) => {
    ...
    if (correct) {
      app.addXp(subject, correct * XP.gameCorrect);
    ...
    <View style={[s.wrap, { paddingTop: Platform.OS === "ios" ? insets.top : insets.top + 6 }]}>
      <View style={s.head}>
        ...
        <View style={{ flex: 1, gap: 4 }}>
          ...
          <Text style={s.phase}>{phaseLabel[phase]} · {level.title}</Text>
        </View>
      ...
      <View style={s.body}>
        ...
        {phase === "result" && result ? (
          <ScrollView contentContainerStyle={[s.scroll, { flexGrow: 1, justifyContent: "center" }]} showsVerticalScrollIndicator={false}>
            ...
            >
              <View style={s.xpRow}>
                <Text style={s.xp}>⚡ +{result.gained} xp</Text>
                {gameScore.total ? <Text style={s.xpMuted}>🧩 gry {gameScore.correct}/{gameScore.total}</Text> : null}
              </View>
      ...
      <TutorModal open={tutor} onClose={() => setTutor(false)} subjectId={subject.id} levelId={level.id} levelTitle={level.title} />
    </View>
  );
}
